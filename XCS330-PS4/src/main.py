...
def cache():
    ...
    for model in models:
        if "num_labels" in model:
            utils.get_model_and_tokenizer(
                model["name"], model["type"], num_labels=model["num_labels"]
            )
        else:
    ...
    for dataset in datasets:
        utils.get_dataset(
            dataset=dataset["name"], n_train=dataset["n_train"], n_val=dataset["n_val"]
        )
...
def run():
    ...
    if args.task == "run_ft":
        ...
        submission.plot_icl(
            args.model.split(","), args.dataset, ks, args.prompt.split(","), args.output
        )
    elif args.task == "plot":
        plot()
